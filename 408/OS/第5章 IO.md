...
> 本节整合两份笔记中关于程序查询/中断/DMA 的内容，并补充通道方式。四种方式的核心演进逻辑是：**CPU 干预频率不断降低、CPU 与外设并行度不断提高**。
...
4.  **保护现场**：在服务程序开头，通过 `PUSH` 指令将通用寄存器的值压入系统堆栈（由程序员编写）。
5.  **设置中断屏蔽字（多重中断核心步 🌟）**：
    -   将当前中断源以及**优先级低于或等于当前中断源**的所有其他中断源的屏蔽位置为 `1`（屏蔽）。
    -   将**优先级高于当前中断源**的屏蔽位置为 `0`（允许开中断后被其打断）。
6.  **开中断（多重中断关键步 🌟）**：执行开中断指令（如 `STI`）。此时，CPU 重新具备响应更高优先级中断的能力。
7.  **执行中断服务程序体**：实际进行数据读写、设备处理。*（若在执行期间有更高优先级的请求到来，CPU 将在此阶段再次进入中断响应阶段，形成中断嵌套）*。
8.  **关中断（多重中断恢复步 🌟）**：在准备恢复现场前，执行关中断指令（如 `CLI`），确保恢复现场和返回原主程序的过程不被新的中断打断。
9.  **恢复现场和中断屏蔽字**：通过 `POP` 指令恢复通用寄存器的原值；恢复原来的中断屏蔽字。
10. **开中断并中断返回**：执行 `IRET` / `RETI` 指令。该指令在硬件上会**自动合并执行两个动作**：从堆栈中弹出原 PC 和 PSW（恢复原主程序断点），同时**重新开中断**，返回原主程序继续执行。
...
- **对物理分区进行<span style="color:red">逻辑格式化</span>**【创建文件系统】，操作系统将初始的文件系统数据结构存储到磁盘上，这些数据结构包括空闲空间和已分配的空间以及一个初始为空的目录
- **簇**：
	- 将多个相邻的扇区组合在一起，提高效率
	- 一簇只能存放一个文件的内容
	- 文件所占用的空间只能是**簇的整数倍**
